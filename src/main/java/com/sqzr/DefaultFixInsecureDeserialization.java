 ...
 *
 */
public class DefaultFixInsecureDeserialization implements FixInsecureDeserialization {
    ...
    public DefaultFixInsecureDeserialization() {
        ...
        String blacklistProperty = System.getProperty("fixinsecuredeserialization.blacklist");
        if(blacklistProperty != null) {
            File blackListFile = new File(blacklistProperty);
            if(!blackListFile.exists()) {
                throw new IllegalArgumentException("Blacklist file specified by 'fixinsecuredeserialization.blacklist' does not exist: " + blackListFile);
            }
            blacklist.addAll(readClassesFromFile(blackListFile));
        }

        String whiteListProperty = System.getProperty("fixinsecuredeserialization.whitelist");
        if(whiteListProperty != null) {
            File whiteListFile = new File(whiteListProperty);
            if(!whiteListFile.exists()) {
                throw new IllegalArgumentException("Whitelist file specified by 'fixinsecuredeserialization.whitelist' does not exist: " + whiteListFile);
        ...
        String dryRunPath = System.getProperty("fixinsecuredeserialization.dryrun");
        if(dryRunPath != null) {
            dryRunWriter = openWriter(dryRunPath);
        }

        String tracePath = System.getProperty("fixinsecuredeserialization.trace");
        if(tracePath != null) {
